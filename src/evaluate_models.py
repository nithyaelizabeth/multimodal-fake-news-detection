...
OUTPUT_PATH = Path("results/tables/model_predictions.csv")
SAMPLE_SIZE = 1000
...
def main():
    ...
    # Select the same balanced subset for every model.
    ...
    test_subset = pd.concat([real_df, fake_df])
    test_subset = test_subset.sample(frac=1, random_state=42)
    ...
    failure_path = Path("results/tables/model_evaluation_failures.csv")
    ...
    Path("results/tables/model_evaluation_coverage.json").write_text(
